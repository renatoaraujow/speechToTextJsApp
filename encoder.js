...
function encodeFlac(audioData){
	...
	if(!Flac.isReady()){
		...
		flacBuffers2.push(audioData);
		console.info('buffered audio data for Flac encoding')
		
	} else {
	
		if(flacBuffers2.length > 0){
			//if there is buffered audio: encode buffered first (and clear buffer)
			
			var len = flacBuffers2.length;
			var buffered = flacBuffers2.splice(0, len);
			for(var i=0; i < len; ++i){
...
function clear(){
	...
	flacLength = 0;
	flacBuffers2.splice(0, flacBuffers2.length);
}
